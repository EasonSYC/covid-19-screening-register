...
let lookModel =     "<tr bgcolor=\"COLOR\">" + 
                    ...
                    "</tr>";
let updateModel =   "document.getElementById(\"loginNUM\").onclick = function() {loginInfo[NUM] = 1 - parseInt(loginInfo[NUM]); updateLook(); check(UA, \"loginInfo\");};" +
                    "document.getElementById(\"liveNUM\").onclick = function() {liveInfo[NUM] = 1 - parseInt(liveInfo[NUM]); updateLook(); check(UA, \"liveInfo\");};" +
                    "document.getElementById(\"workNUM\").onclick = function() {workInfo[NUM] = 1 - parseInt(workInfo[NUM]); updateLook(); check(UA, \"workInfo\");};" +
                    "document.getElementById(\"moveNUM\").onclick = function() {moveInfo[NUM] = 1 - parseInt(moveInfo[NUM]); updateLook(); check(UA, \"moveInfo\");};";
...
function updateLook() {
    ...
    for (let i = 0; i < buildingInfo.length; ++i) {
        ...
        let j = 0;
        if (str(loginInfo[i]) === "1") {
            j = 1;
        } else if (str(liveInfo[i]) === "1") {
            j = 2;
        } else if (str(workInfo[i]) === "1") {
            j = 3;
        } else if (str(moveInfo[i]) === "1") {
            j = 4;
        }
